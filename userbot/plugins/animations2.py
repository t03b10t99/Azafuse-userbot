...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Azafuse User"
...
async def _(event):
    ...
    animation_chars = [
        "**Heroku Connecting To Latest [Github Build](dicyn/Azafuse-Userbot)**",
...
@bot.on(admin_cmd(pattern="nowar$"))
@bot.on(sudo_cmd(pattern="nowar$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    event = await edit_or_reply(event, "nowar")
    await event.edit("WO")
